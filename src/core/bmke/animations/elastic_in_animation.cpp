...
#include "elastic_in_animation.hpp"
#include <cassert>
#include "../entity.hpp"

namespace bmonkey{

ElasticInAnimation::ElasticInAnimation(const float delay, const float duration):
...
ElasticInAnimation::~ElasticInAnimation(void)
{
...
void ElasticInAnimation::run(void)
{
	float dest_pos;
	...
	delete m_tween;
	m_tween = new CDBTweener::CTween(&CDBTweener::TWEQ_ELASTIC, CDBTweener::TWEA_OUT, getDuration(), &m_current_pos, dest_pos);
	m_clock.restart();
}

void ElasticInAnimation::update(sf::Time delta_time)
{
