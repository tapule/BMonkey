...
#include "back_in_animation.hpp"
#include <cassert>
#include "../entity.hpp"

namespace bmonkey{

BackInAnimation::BackInAnimation(const float delay, const float duration):
...
void BackInAnimation::run(void)
{
	...
	delete m_tween;
	m_tween = new CDBTweener::CTween(&CDBTweener::TWEQ_BACK, CDBTweener::TWEA_OUT, getDuration(), &m_current_pos, dest_pos);
	m_clock.restart();
}

void BackInAnimation::update(sf::Time delta_time)
{
