...
#include "fade_out_animation.hpp"
#include <cassert>
#include "../entity.hpp"

namespace bmonkey{

FadeOutAnimation::FadeOutAnimation(const float delay, const float duration):
...
FadeOutAnimation::~FadeOutAnimation(void)
{
}

void FadeOutAnimation::init(Entity* entity)
{
	Animation::init(entity);
	m_entity_color = entity->getColor();
}

void FadeOutAnimation::run(void)
{
...
void FadeOutAnimation::stop(void)
{
	m_finished = true;
	m_entity->setColor(m_entity_color);
}

void FadeOutAnimation::update(sf::Time delta_time)
{
